...
namespace SPlugin.RemoteConsole.Runtime
{
    internal class CommandItemLong : CommandItemAbstract
    {
        ...
        public override VisualElement CreateUIToolkitControl()
        {
            ...
            longField.AddToClassList("command-input-field");
            UIResourceHelper.ApplyDefaultFont(longField);
            
            // Apply default font - CSS handles styling
            longField.schedule.Execute(() => {
                // Try multiple possible input element class names
                var textInput = longField.Q(className: "unity-text-input") ?? 
                               longField.Q(className: "unity-base-text-field__input") ?? 
                               longField.Q(className: "unity-text-field__input");
                if (textInput != null)
                {
                ...
                var textElement = longField.Q(className: "unity-text-element");
                if (textElement != null)
                {
